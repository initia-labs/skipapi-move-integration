module skip::entrypoint {	
	...
	struct ModuleStore has key {
		ibc_transfer_adaptor: AdaptorInfo,
	...
	fun init_module(chain: &signer) {
		let ibc_transfer_adaptor = AdaptorInfo {
			module_address: @skip,
			module_name: string::utf8(b"ibctransfer"),
		};
		...
		move_to(chain, ModuleStore {
			ibc_transfer_adaptor: ibc_transfer_adaptor,
...
// Note: entry functions can take primitive types, String, and vector arguments but cannot take Structs (e.g. Option). They also must not have any return values.
	...
	public fun default_adaptors_test(
		...
		let ibc_transfer_adaptor = module_store.ibc_transfer_adaptor;
		assert!(ibc_transfer_adaptor.module_address == @skip, 1);
		assert!(ibc_transfer_adaptor.module_name == string::utf8(b"ibctransfer"), 1);
