module skip::initia_dex {
    ...
    public fun get_spot_price(
        ...
        while (i < swap_length) {
            ...
            spot_price = bigdecimal::mul(
                spot_price, dex::get_spot_price_by_denom(*pair, *coin_in_metadata)
            );
            i = i + 1;
        };
    ...
    #[test_only]
    fun initialized_module_for_test2(chain: &signer): (vector<String>, vector<String>) {
        dex::init_module_for_test();
        primary_fungible_store::init_module_for_test();
        ...
        (
            vector[coin::metadata_to_denom(lp_metadata)],
            vector[
                coin::metadata_to_denom(usdc_metadata),
                coin::metadata_to_denom(init_metadata)
            ]
        )
    }
    ...
    fun test_spot_price(chain: signer) {
        ...
        let spot_price = get_spot_price(pools, coins);
        assert!(
            bigdecimal::eq(spot_price, bigdecimal::from_ratio_u64(4, 1)),
            0
        );

        let price = bigdecimal::one();
        let spot_price =
            bigdecimal::from_scaled(biguint::from_u128(6117326889893584277078));
        let price = bigdecimal::mul(price, spot_price);
        assert!(price == spot_price, 0);
    }
    ...
    fun test_swap_exact_asset_in(chain: signer) {
        ...
        assert!(
            coin::balance(chain_addr, *vector::borrow(&coins, 0)) == before_coin0
                - 1000,
            0
        );
        assert!(
            coin::balance(chain_addr, *vector::borrow(&coins, 1)) == before_coin1 + 996,
            1
        );
    }
    ...
    fun test_swap_exact_asset_out(chain: signer) {
        ...
        assert!(
            coin::balance(chain_addr, *vector::borrow(&coins, 1)) == before_coin1 + 995,
            1
        );
    }
    ...
    fun test_simulate_swap_exact_asset_out(chain: signer) {
        ...
        assert!(
            expected_amount >= 999 && expected_amount <= 1001,
            0
        );
    }
}
